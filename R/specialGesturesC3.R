c3 <- function(xStart, yStart, headingStart, method = "bezier"){
  ...
  if (method == "bezier") {
    ...
    path = data.frame(list(path1 = path1, path2 = path2, path3 = path3, path4 = path4))

    path <- as.matrix(cbind(c(path[, 1], rev(path[, 1]),
                              path[, 3], rev(path[, 3]),
                              path[, 5], rev(path[, 5]),
                              path[, 7], rev(path[, 7])),
                            c(path[, 2], rev(path[, 2]),
  ...
  controlPoints <- rbind(controlPoints1, controlPoints2, controlPoints3, controlPoints4)
